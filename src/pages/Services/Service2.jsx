import "./Service2.css";

const Service2 = () => {
  return (
    <>
      <div className="Services-container" id="services">
        <p className="heading">Services</p>
        <h2 className="second-heading">
          Have a High Impact & World Class{" "}
          <span> LinkedIn Growth Strategy </span>
        </h2>
        <p
          style={{ padding: "0px 10px 20px 10px", textTransform: "capitalize" }}
        >
          Here’s How We help with LinkedIn
        </p>
        <div className="service-sec">
          <div className="inner-ser">
            <div className="ser-heading">Strategic Brand Positioning</div>
            <div className="ser-text">
              <p>
                With so many people sharing content on LinkedIn, how can you
                make sure you stand out? It all starts with really knowing your
                business and yourself.
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  This includes your unique stories, vision, value you have to
                  offer, and insights that set you apart.
                </span>
              </p>
              <br></br>
              <p>- Offer creation</p>
              <p>- Brand voice alignment</p>
              <p>- Landing page consultation</p>
              <p>- LinkedIn profile optimization</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">World-Class Copywriting</div>
            <div className="ser-text">
              <p>
                On LinkedIn, every word counts. If your post’s hook, formatting,
                and concept don’t immediately engage the reader, they’re
                destined to keep scrolling.
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  Your posts must grab attention and keep the audience reading
                  until the end. We'll handle all that.
                </span>
              </p>
              <br></br>
              <p>- Value-focused ideation</p>
              <p>- Content strategy creation</p>
              <p>- Top-notch posts copywriting</p>
              <p>- Ongoing feedback and refinement</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">Expert Graphic Design</div>
            <div className="ser-text">
              <p>
                Visual appeal is key, and We truly understand. We will create
                detailed infographics,{" "}
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  vibrant post visuals, and dynamic carousels that will
                  captivate your target audience and leave a lasting impression.
                </span>
              </p>
              <br></br>
              <p>- Attention-grabbing graphics</p>
              <p>- Custom audience-centric banners</p>
              <p>- Engaging cheatsheets and carousels</p>
              <p>- A well-balanced brand image</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">Hands-off Publishing</div>
            <div className="ser-text">
              <p>
                We don’t just manage your content, We also handle the
                publishing. Each post is released on the optimal day and time to
                maximize attention from your target audience.
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  We’ll even repurpose your top-performing content to further
                  boost your visibility.
                </span>
              </p>
              <br></br>
              <p>- Performance analysis</p>
              <p>- Conversion-optimized copywriting</p>
              <p>- Strategic content repurposing</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">Networking strategy</div>
            <div className="ser-text">
              <p>
                We're happy to implement our proven LinkedIn networking strategy
                on your account, which has generated thousands of impressions
                and potential warm leads.{" "}
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  We dedicate 1 hour daily (5 days a week) to executing this
                  strategy on your behalf, leading to:{" "}
                </span>
              </p>
              <br></br>
              <p>- Building thought leadership</p>
              <p>- Increasing brand awareness</p>
              <p>- Generating more inbound leads</p>
              <p>- Receiving potential collaboration opportunities</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">Ongoing Consulting</div>
            <div className="ser-text">
              <p>
                A good LinkedIn strategy keeps up with the times. As your
                industry, the platform, and the needs of your target market
                evolve,{" "}
                <span style={{ fontWeight: "bolder" }}>
                  we’ll be right there, ensuring you’re always on top of things.{" "}
                </span>
              </p>
              <br></br>
              <p>- 1-1 consultation every week</p>
              <p>- Ongoing feedback and refinement</p>
              <p>- Know about top LinkedIn trends</p>
              <p>- Keep evolving your LinkedIn brand</p>
            </div>
          </div>
          <hr className="hr" />
          <div className="inner-ser">
            <div className="ser-heading">ADD-ON SERVICES (Optional)</div>
            <div className="ser-text">
              <p>
                To ensure maximum lead flow,
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  We offer few add-on services to reach and engage your target
                  market proactively.{" "}
                </span>
                This is guided by thousands of hours of experience in building
                relationships and lead prospecting.
              </p>
              <br></br>
              <p>- LinkedIn newsltter</p>
              <p>- Personalized messaging outreach</p>
              <p>- Content repurposing on Meta, X, Instagram</p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
export default Service2;
